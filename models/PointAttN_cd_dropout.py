...
class cross_transformer(nn.Module):
    def __init__(self, d_model=256, d_model_out=256, nhead=4, dim_feedforward=1024, dropout=0.1):
        super().__init__()
        print(f"cross_transformer: d_model={d_model}, d_model_out={d_model_out}, nhead={nhead}, dim_feedforward={dim_feedforward}, dropout={dropout}")
...
class Model(nn.Module):
    ...
    def forward(self, src, tgt=None, models=None, is_training=True):
        ...
        # print(f"src shape: {src.shape}, tgt shape: {tgt.shape if tgt is not None else 'None'}, models shape: {models.shape if models is not None else 'None'}")
        ...
        fine, feat_fine= self.refine(None, new_x, src_embeddings)
        fine1, feat_fine1 = self.refine1(feat_fine, fine, src_embeddings)
        ...
        # print(f"gt Shape {tgt.shape, tgt_fine.shape}, Fine shape: {fine.shape}, Fine1 shape: {fine1.shape}, Coarse shape: {coarse.shape}, Input shape {src.shape}")
