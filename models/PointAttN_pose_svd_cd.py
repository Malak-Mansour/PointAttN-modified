...
class MLPHead(nn.Module):
    def __init__(self, emb_dims):
        ...
        # Change input dimension from emb_dims * 2 to emb_dims
    ...
    def forward(self, src_embedding):
        ...
        # Remove tgt_embedding and concatenation
...
class Model(nn.Module):
    ...
    def forward(self, src, tgt=None, models=None, is_training=True):
        ...
        models_sampled = self.sample_points(models, src.shape[2])
        assert not torch.isnan(models_sampled).any(), f"models_sampled contains NaN values: {torch.isnan(models_sampled).any()}"
        src_embedding, coarse = self.encoder(src)      
        assert not torch.isnan(src_embedding).any(), f"src_embedding contains NaN values: {torch.isnan(src_embedding).any()}"
        tgt_embedding, _ = self.encoder(models_sampled.transpose(1, 2).contiguous())
        assert not torch.isnan(tgt_embedding).any(), f"tgt_embedding contains NaN values: {torch.isnan(tgt_embedding).any()}"


        new_x = torch.cat([src,coarse],dim=2)
        new_x = gather_points(new_x, furthest_point_sample(new_x.transpose(1, 2).contiguous(), 512))

        fine, feat_fine = self.refine(None, new_x, src_embedding)
        fine1, feat_fine1 = self.refine1(feat_fine, fine, src_embedding)

        coarse = coarse.transpose(1, 2).contiguous()
        tgt_coarse = self.sample_points(tgt, coarse.shape[1])
        ...
        # print(f"Input Shape {src.shape}, Fine shape: {fine.shape}, Fine1 shape: {fine1.shape}, Coarse shape: {coarse.shape}")
       
        # src_embedding_p, tgt_embedding_p = self.pointer(src_embeddings_local, tgt_embeddings_local)
        
        # src_embeddings_local = (src_embeddings_local + src_embedding_p).transpose(1, 2).contiguous()
        # tgt_embeddings_local = (tgt_embeddings_local + tgt_embedding_p).transpose(1, 2).contiguous()

        
        # Rs_pred, ts_pred = self.pose_head(src_embeddings_local, tgt_embeddings_local, src, models_sampled.transpose(1, 2).contiguous())
        ...
        models_sampled = self.sample_points(models, src_embedding.shape[2]).transpose(1, 2).contiguous()
        src_sampled = self.sample_points(src.transpose(1, 2).contiguous(), src_embedding.shape[2]).transpose(1, 2).contiguous()

        Rs_pred, ts_pred = self.pose_head(src_embedding, tgt_embedding, src_sampled, models_sampled)
        ...
        if is_training:
            ...
            if enable_hypercd:
                ...
                loss4, _ = calc_cd(models_transformed, tgt)
                ...
                loss1, _ = calc_cd(coarse, tgt_coarse)
            ...
            # Call get_loss1 with appropriate parameters
                ...
                cd_p_coarse, cd_t_coarse = calc_cd(coarse, tgt_coarse)
                cd_p_models, cd_t_models = calc_cd(models_transformed, tgt)

                return {'out1': coarse, 'out2': fine1, 'cd_t_coarse': cd_t_coarse, 'cd_p_coarse': cd_p_coarse, 'cd_p': cd_p, 'cd_t': cd_t, 'cd_p_models': cd_p_models, 'cd_t_models': cd_t_models}
            else:
