...
from models.mv_utils_zs import *
...
class ResEncoder(nn.Module):
    ...
    def forward(self, input_view):
        if input_view.size(1) == 1:
            input_view = input_view.repeat(1, 3, 1, 1)
...
class Model(nn.Module):
    ...
    def forward(self, src, depths, tgt=None, models=None, is_training=True):
        ...
        feat_g, coarse = self.encoder(src, depths)
        local_feat = self.localencoder(src)
        
        coarse_merge = torch.cat([src,coarse],dim=2)
        coarse_merge = gather_points(coarse_merge, furthest_point_sample(coarse_merge.transpose(1, 2).contiguous(), self.merge_points))

        fine,F_L_1 = self.refine1(local_feat, coarse_merge, feat_g,src)
        fine1 = self.refine2(local_feat, fine, feat_g,src,F_L_1)
