...
from cfgs.config_pcn import cfg
def train():
    logging.info(str(args))
    metrics = ['cd_p', 'cd_t', 'cd_t_coarse', 'cd_p_coarse', "cd_hyp", "f1", "cd_p_models", "cd_t_models"]
    ...
    data_dir = './data_files/data_fighter'

# Training dataset with augmentation
    dataset = PCDDataset(data_dir, augment=True)
    ...
    data_dir = './data_files/data_fighter_validation/'
    ...
    for epoch in range(args.start_epoch, args.nepoch):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            gt = gt.float().cuda()
            models = models.float().cuda()
            
            partial_depth = torch.unsqueeze(render.get_img(inputs),1)
            
            
            inputs = inputs.transpose(2, 1).contiguous()
    
            out2, loss2, net_loss = net(inputs, partial_depth, gt, models)
            ...
            if has_nan_grad:
                logging.error(f"Skipping optimizer step due to NaN gradients at iteration {i}")
                optimizer.zero_grad()
                continue
            ...
            if torch.isnan(grad_norm) or torch.isinf(grad_norm):
                logging.error(f"NaN/Inf gradient norm at iteration {i}")
                optimizer.zero_grad()
                continue
            
                    # ---- Add this line to check for exploding gradients ----
...
def val(net, curr_epoch_num, val_loss_meters, dataloader_test, best_epoch_losses):
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader_test):
            ...
            result_dict = net(inputs, partial_depth,  gt, models, is_training=False)
